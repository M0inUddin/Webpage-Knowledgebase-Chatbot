        ...
        try:
            ...
            if not force_refresh:
                ...
                if collection_info.get("count", 0) > 0:
                    logger.info(
                        f"Using existing knowledge base with {collection_info['count']} documents"
                    )
                    return True
            ...
            logger.info(
                f"Knowledge base initialized successfully with {len(all_chunks)} chunks"
            )
            return True
        ...
        try:
            ...
            if not retrieved_documents:
                ...
                prompt = self.prompt_engineering.create_rag_prompt(
                    query_type, query, context, conversation_history
                )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--refresh", action="store_true", help="Force refresh of the knowledge base"
    )
    parser.add_argument(
        "--initialize-only",
        action="store_true",
        help="Only initialize the knowledge base, then exit",
    )
